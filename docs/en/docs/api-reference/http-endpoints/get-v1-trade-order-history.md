---
title: GET /v1/trade/order/history
id: get-v1-trade-order-history
slug: /http-endpoints/get-v1-trade-order-history
sidebar_position: 10
---

## Request

- **HTTP Method:** `GET`
- **HTTP URL:** `/v1/trade/order/history`

## Base URL

- `https://openapi.longportapp.com`
- `https://openapi.longportapp.cn`

## Full URL Example

```text
https://openapi.longportapp.com/v1/trade/order/history
```

## Source (SDK mapping)

- `rust/src/trade/context.rs`
- Repository: <https://github.com/longportapp/openapi/tree/main/rust/src>

> This page is generated from SDK source mapping and is intended as HTTP path reference.
