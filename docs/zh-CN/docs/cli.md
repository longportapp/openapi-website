...
```bash
longbridge filings AAPL.US [--count 20]          # 公司公告与监管文件列表（财报、公告、SEC 申报等）
...
```bash
longbridge my-topics                           # 我发布的全部讨论
longbridge my-topics --type article            # 仅长文
longbridge my-topics --type post --size 10     # 短帖，每页 10 条
longbridge my-topics --page 2                  # 翻页
longbridge create-topic --body "今天看好 700.HK"                                           # 发短帖（纯文本）
longbridge create-topic --body "NVDA GTC 看点" --tickers NVDA.US                          # 带关联标的
longbridge create-topic --title "我的分析" --body "$(cat post.md)" --type article          # 发长文（从文件读取）
```
