...
```bash
longbridge filings AAPL.US [--count 20]          # 公司公告與監管文件列表（財報、公告、SEC 申報等）
...
```bash
longbridge my-topics                           # 我發布的全部討論
longbridge my-topics --type article            # 僅長文
longbridge my-topics --type post --size 10     # 短帖，每頁 10 條
longbridge my-topics --page 2                  # 翻頁
longbridge create-topic --body "今天看好 700.HK"                                           # 發短帖（純文本）
longbridge create-topic --body "NVDA GTC 看點" --tickers NVDA.US                          # 帶關聯標的
longbridge create-topic --title "我的分析" --body "$(cat post.md)" --type article          # 發長文（從文件讀取）
```
