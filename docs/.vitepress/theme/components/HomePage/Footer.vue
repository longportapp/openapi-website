<template>
  <footer class="footer">
    <div class="flex flex-wrap items-center text-xs gap-x-5 gap-y-2">
      ...
      <a v-for="link in leftLinks" :key="link.href" :href="link.href" class="inline-block footer-link" :target="link.href.startsWith('/') ? '_self' : '_blank'">
        {{ link.label }}
      </a>
    </div>
    <div class="flex flex-wrap items-center text-xs gap-x-5 gap-y-2">
      <a v-for="link in rightLinks" :key="link.href" :href="link.href" class="inline-block footer-link" :target="link.href.startsWith('/') ? '_self' : '_blank'">
        {{ link.label }}
      </a>
...
const leftLinks = computed(() => [
  ...
  { href: `https://support.longbridgewhale.com/topics/misc.disable/lp-user-agreement?locale=${locale.value}`, label: t('side-footer.section-terms.terms-of-service') },
  { href: `https://support.longbridgewhale.com/topics/misc/privacy-policy?locale=${locale.value}`, label: t('side-footer.section-terms.privacy-policy') },
])
