...
const CLIENT_H = 34
const BUS_TOP_Y = 82
const HUB_X = 240, HUB_Y = 108, HUB_W = 160, HUB_H = 40
const HUB_CX = HUB_X + HUB_W / 2  // 320
const BUS_BOT_Y = 192
const TOOL_Y = 218, TOOL_H = 78
    ...
    <ClientOnly>
      <div class="mcp-diagram-wrap">
        ...
        >
          ...
          <line
            class="mcp-edge"
            :x1="clientCenters[0]" :y1="BUS_TOP_Y"
            :x2="clientCenters[clientCenters.length - 1]" :y2="BUS_TOP_Y"
          />
          <!-- Active bus segment: active client → hub center (highlighted overlay) -->
          <line
            class="mcp-edge mcp-edge-hi"
            :x1="clientCenters[activeClient]" :y1="BUS_TOP_Y"
            :x2="HUB_CX" :y2="BUS_TOP_Y"
          />

          <!-- Client stubs: each client bottom → top bus -->
          <line
            v-for="(cx, i) in clientCenters" :key="`cs-${i}`"
          ...
          <line
            ...
            :x1="toolCenters[0]" :y1="BUS_BOT_Y"
            :x2="toolCenters[toolCenters.length - 1]" :y2="BUS_BOT_Y"
          />

          <!-- Tool stubs: bottom bus → each tool top (branded) -->
          <line
            v-for="(tx, i) in toolCenters" :key="`ts-${i}`"
            class="mcp-edge mcp-edge-down"
            :x1="tx" :y1="BUS_BOT_Y"
            :x2="tx" :y2="TOOL_Y"
          />
          ...
          <!-- Client nodes -->
          <foreignObject
            v-for="(node, i) in clientNodes" :key="`c-${i}`"
            :x="node.x" :y="0" :width="node.w" :height="CLIENT_H"
          >
            <div
          ...
          <!-- Tool nodes -->
          <foreignObject
            v-for="(pos, i) in toolPositions" :key="`t-${i}`"
            :x="pos.x" :y="TOOL_Y" :width="pos.w" :height="TOOL_H"
          >
...
<style scoped>
.mcp-section { padding: 4rem 1rem; background: var(--vp-c-bg-soft); overflow: hidden; }

.mcp-header { text-align: center; margin-bottom: 2rem; padding: 0 1.5rem; max-width: 48rem; margin-left: auto; margin-right: auto; box-sizing: border-box; width: 100%; }
.mcp-title { font-size: 1.75rem; font-weight: 700; color: var(--vp-c-text-1); letter-spacing: -0.02em; }
.mcp-subtitle { margin-top: 1.5rem; font-size: 0.875rem; color: var(--vp-c-text-2); font-weight: 500; line-height: 1.4; }
.mcp-desc { margin-top: 0.375rem; font-size: 0.78rem; line-height: 1.6; color: var(--vp-c-text-3); }
...
@keyframes mcp-dash { to { stroke-dashoffset: -8; } }
...
.mcp-n-client:hover:not(.more) { border-color: var(--brand-color); }
.mcp-n-client.active {
...
.mcp-n-tool-title { font-size: 0.78rem; font-weight: 700; color: var(--vp-c-text-1); line-height: 1.3; }
.mcp-n-tool-desc {
  font-size: 0.65rem;
  color: var(--vp-c-text-3);
  line-height: 1.4;
...
/* Config panel */
.mcp-config { max-width: 64rem; margin: 0 auto; padding: 0 1.5rem; }
...
.mcp-copy-btn { display: flex; align-items: center; justify-content: center; width: 1.5rem; height: 1.5rem; border-radius: 0.25rem; color: var(--vp-c-text-3); background: transparent; border: none; cursor: pointer; transition: all 0.2s; flex-shrink: 0; }
.mcp-copy-btn:hover { color: var(--brand-color); background: color-mix(in srgb, var(--brand-color) 8%, transparent); }
.mcp-copy-btn:active { transform: scale(0.8); }

.mcp-config-note { margin-top: 0.5rem; font-size: 0.72rem; color: var(--vp-c-text-3); text-align: center; }
