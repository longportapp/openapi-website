...
<template>
  ...
  >
    <div class="absolute inset-0 -z-30 overflow-visible blur-[2px]">
      <div class="animate-shimmer-btn-shimmer-slide absolute inset-0 aspect-square h-full rounded-none">
        <div
          class="animate-shimmer-btn-spin-around absolute -inset-full w-auto rotate-0 [background:conic-gradient(from_calc(270deg-(var(--spread)*0.5)),transparent_0,var(--shimmer-color)_var(--spread),transparent_var(--spread))]"
        />
      </div>
    </div>
    <slot />
    ...
    />
    <div class="absolute -z-20 [background:var(--bg)] [border-radius:var(--radius)]" :style="{ inset: 'var(--cut)' }" />
  </button>
</template>
