...
onMounted(() => {
  intervalId = setInterval(() => {
    ...
    if (document.visibilityState === 'visible') {
      if (Date.now() - lastHidden.value > 500) count.value++
    } else {
...
<template>
  <motion.span
    ...
    :transition="{ duration: props.duration, delay: index * 0.05 }"
    :animate="{
      y: [0, -3, 0],
      opacity: [1, 0.8, 1],
      scale: [1, 1.01, 1],
    ...
    :exit="{ y: -3, opacity: 1, scale: 1, filter: 'blur(5px)', color: props.startColor }"
  >
    {{ char }}
  </motion.span>
</template>
