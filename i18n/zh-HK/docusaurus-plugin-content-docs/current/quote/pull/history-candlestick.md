...
| ∟ start_date   | string | 否       | 開始日期，格式為 `YYYYMMDD`，例如：20231016 <br /><br />**參數說明：**<br /> start_date 和 end_date 均不填： 返回最新的 1000 根 K 線； <br />僅填 start_date： 返回 start_date 與最新交易日區間內的 K 線。若此區間內 K 線超過 1000 根，則優先返回靠近 start_date 的 1000 根 K 線；<br /> 僅填 end_date ： 返 回 end_date 及以前的 1000 根 K 線；<br />start_date 和 end_date 均填：返回此區間內的 K 線數據。若次區間內 K 線超過 1000 根，則優先返回靠近 end_date 的 1000 根 K 線 |
...
依據用戶在長橋的資産和交易情況，不同類型的用戶每月可查詢曆史數據的標的數量如下錶：

- 額度按照自然月計算，每月初額度加滿，上月剩餘額度不纍計到本月。一個自然月內重複請求同一隻標的的曆史 K 線，僅統計一次。
- 新入金的賬戶，額度會在下個交易日自動生效；當賬戶的總資産或交易筆數增加、且達到更高等級時，額度會在下一個交易日生效。
- 總資産：用戶在長橋的港股、美股、A 股等證券賬戶的總資産，按照匯率換算成港元。取用戶上個自然月最後一個交易日的總資産與最近一個完整交易日的總資産的較大值。
...
| -------- | ---------------- | --------------------- | ------------------------------------------------------------------------ |
| 港股     | 2004-6-1 至今    | 2022-09-28 09:30:00   |                                                                          |
| 美股     | 2010-6-1 至今    | 最近 6 個交易日的數據 | 美股曆史分鍾 K 目前支持查詢近 6 個交易日的數據，待後續開放更長時段的數據 |
