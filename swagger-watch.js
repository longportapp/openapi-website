...
function generateSwaggerDoc(file) {
  ...
  const locale = fileName.match(/openapi\/(en|zh-HK|zh-CN)/)?.[1]
  ...
  try {
    ...
    let distPath = fileName.replace('openapi', 'docs').replace(/\.(yml|yaml)/, '--autogen.md')
...
function generateAllDocs() {
  glob('./openapi/**/*.yml', function (err, files) {
    files.forEach((file) => {
